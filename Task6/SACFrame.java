				...
				*/
				...
				if (memInCacheBlock == -1){
					...
					if (!(emptyCacheBlock == -1)){ //AN EMPTY BLOCK IS AVAILABLE WITHIN THE SET
						...
						lruCacheBlock = getLFUCacheBlock(numWaysSel, intSetDec);
...
//		moveStatus = 0;
	...
	//FUNCTION TO RETURN THE INDEX OF THE First BLOCK IN THE SET	
	public int getFIFOCacheBlock(int numWays, int set){
		
		//INTEGER TO HOLD THE VALUE OF THE INDEX WITH THE FIRST VALUE
		int first = 0;
	...
	}//END FUNCTION getFIFOCacheBlock
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //FUNCTION TO RETURN THE INDEX OF THE LFU BLOCK IN THE SET	
	public int getLFUCacheBlock(int numWays, int set){
		
		//INTEGER TO HOLD THE VALUE OF THE INDEX WITH THE LEAST LRU VALUE
		int maximum = 0;

		if (numWays == 0){
			//COMPARE THE VALUES OF THE ARRAY statusCacheLRU2Way AND RETURN THE INDEX WITH THE MAXIMUM VALUE
			if (statusCacheLRU2Way[set][0] < statusCacheLRU2Way[set][1])
				maximum = 1;			
		}
		else {
			//COMPARE THE VALUES OF THE ARRAY statusCacheLRU4Way AND RETURN THE INDEX WITH THE MAXIMUM VALUE
			for (int i = 0; i < 4; i++){
				if (statusCacheLRU4Way[set][minimum] < statusCacheLRU4Way[set][i])
					maximum = i;
			}
