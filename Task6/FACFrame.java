			...
			case 3:	//FIND THE CACHE BLOCK IN QUESTION AND HIGHLIGHT IT		
				...
				if (memInCache == -1){
					...
					//ELSE DISPLAY THE LRU CACHE BLOCK WHICH WILL BE REPLACED
						...
						lruCacheBlock = getLFUCacheBlock();
...
//			listData[i] = "";
	...
	//FUNCTION TO RETURN THE INDEX OF THE FIRST USED CACHE BLOCK

	public int getFIFOCacheBlock(){

		//INTEGER TO HOLD THE VALUE OF THE FIRST INDEX 
		int first = 0;
	...
	public int getLFUCacheBlock(){

		//INTEGER TO HOLD THE VALUE OF THE INDEX WITH THE LEAST LRU VALUE
		int maximum = 0;

		//COMPARE THE VALUES OF THE ARRAY statusCacheLRU AND RETURN THE INDEX WITH THE MAXIMUM VALUE
		for (int i = 1; i < 16; i++){
			if (statusCacheLRU[maximum] < statusCacheLRU[i])
				maximum = i;
		}
		return maximum;
