...
class VMFrame extends JFrame {
				...
				if (tlbEntry >= 0){
					...
					if (pageTableEntry >= 0){//THERE WAS A PAGE HIT
						...
						if (!(emptyTLBRow == -1)){		//IF THERE IS AN EMPTY TLB ROW				
							...
							tlbPanel.virtPageNum[getLFURow()] = ""+intVirtPageDec;
							tlbPanel.physPageNum[getLFURow()] = ""+pageTableEntry;
						
							//UPDATE THE LRU STATUS OF THE TLB ROW
							statusRowLRU++;
							statusTLBRowLRU[getLFURow()] = statusRowLRU;
			...
			case 6: //BRING THE PAGE INTO MEMORY AND HIGHLIGHT THE FRAME WITH THE DATA IN IT
				...
				if (boolTLBHit || boolPageHit){
					...
					if (!(emptyTLBRow == -1)){ //AN EMPTY TLB ROW IS AVAILABLE	
						...
						tlbPanel.virtPageNum[getLFURow()] = ""+intVirtPageDec;
						tlbPanel.physPageNum[getLFURow()] = ""+frameNum;
						tlbPanel.boolRows[getLFURow()] = true;
	...
	*/
	...
	//FUNCTION TO RETURN THE INDEX OF THE FIRST TLB ROW

	public int getFIFORow(){
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //FUNCTION TO RETURN THE INDEX OF THE LEAST FREQUENTLY USED TLB ROW

	public int getLFURow(){

		//INTEGER TO HOLD THE VALUE OF THE INDEX WITH THE MAX LFU VALUE
		int maximum = 0;

		//COMPARE THE VALUES OF THE ARRAY statusTLBRowLRU AND RETURN THE INDEX WITH THE MAXIMUM VALUE
		for (int i = 1; i < 4; i++){
			if (statusTLBRowLRU[minimum] < statusTLBRowLRU[i])
				maximum = i;
		}
